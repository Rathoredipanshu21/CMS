...
// 4. NEW: Fetch Recent Transactions (Corrected Query)
$recent_transactions_sql = "SELECT company_name, final_payable_amount, transaction_date FROM transactions ORDER BY id DESC LIMIT 5";
$recent_transactions_result = $conn->query($recent_transactions_sql);

// Recommended: Add error checking for the query
if ($recent_transactions_result === false) {
    // This makes debugging easier by showing the exact SQL error
                    ...
                    <?php if ($recent_transactions_result->num_rows > 0): ?>
                        <?php while($row = $recent_transactions_result->fetch_assoc()): ?>
                            <div class="flex items-center">
                                ...
                                <div>
                                    <p class="font-semibold text-gray-800"><?php echo htmlspecialchars($row['company_name']); ?></p>
